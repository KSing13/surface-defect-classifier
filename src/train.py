...
base_model = tf.keras.applications.MobileNetV2(include_top=False, input_shape=(*config.IMG_SIZE, 3), weights='imagenet')
base_model.trainable = False

model = tf.keras.Sequential([
    tf.keras.layers.InputLayer(input_shape=(*config.IMG_SIZE, 3)),
    data_augmentation,
    tf.keras.layers.Rescaling(1./127.5, offset=-1), # Preprocessing for MobileNetV2 [-1, 1]
    base_model,
